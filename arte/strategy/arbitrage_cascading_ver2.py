...
class SignalState:
    ...
    def premium_overshoot_min(self, **kwargs):
        zscore = self.update_zscore(**kwargs)
        #print(zscore[-5:])
        return zscore[-1] >= (self.order_state +1)
    ...
    def buy_long(self, **kwargs):
        ...
        #premium_dif = kwargs["premium_q"][-1] - kwargs["criteria_premium_q"][-1]
        #print(f"current kimp diff : {premium_dif } \n zscore : {kwargs["zscore"]} \n ")
    ...
    def premium_decrease(self, **kwargs):
        zscore = self.update_zscore(**kwargs)
        return zscore[-1] <= (self.order_state-0.5)
    ...
    def update_zscore(self, **kwargs):
        kimp_q = np.array(kwargs["premium_q"]) - np.array(kwargs["criteria_premium_q"])
