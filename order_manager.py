...
class OrderManager:
            ...
            if o.type == "LIMIT":
                if o.status == "NEW":
                    ...
                    if o.status != result.status:
                        print(f"""Status Changed - Order {result.clientOrderId}: {o.status} -> {result.status} /
    ...
    def long_limit(self, price: float, quantity: float):
        result = self.request_client.post_order(symbol=self.symbol, 
                                                side=OrderSide.BUY, 
    ...
    def buy_long_limit(self, price, quantity=None, ratio=None):
        ...
        if bool(quantity) ^ bool(ratio):
            if ratio: quantity = self._usdt_ratio_to_quantity(ratio, price)
    ...
    def sell_long_limit(self, price, quantity=None, ratio=None):
        ...
        '''
        if bool(quantity) ^ bool(ratio):
            if ratio: quantity = self._symbol_ratio_to_quantity(ratio)
            if self.account.positions[self.symbol].positionAmt > 0:
    ...
    def buy_long_market(self, quantity=None, ratio=None):
        ...
        '''
        if bool(quantity) ^ bool(ratio):
            if ratio: quantity = self._usdt_ratio_to_quantity(ratio, self.get_ticker_price())
            if self.account.positions[self.symbol].positionAmt >= 0:
    ...
    def _usdt_ratio_to_quantity(self, ratio, price, unit_float=3):
        return math.floor(((self.account.usdt_balance * ratio) / price) * (10**unit_float)) / (10**unit_float) # cut 3 decimal under floating point
